...
envFile.split('\n').forEach(line => {
  if (line && line.includes('=')) {
    ...
    const v = parts.join('=').trim().replace(/^"|"$/g, '').replace(/^'|'$/g, '');
    envVars[k] = v;
  }
});
...
(async () => {
  ...
  const { error: e1 } = await supabase.from('cms_policies').insert([{
    type: 'privacy',
    content: fallbackPrivacyText,
    version: 1,
    is_published: true
  }]);
  
  if (e1) console.error("Error inserting privacy:", e1);
  else console.log("Privacy policy inserted.");

  console.log("Inserting terms...");
  const { error: e2 } = await supabase.from('cms_policies').insert([{
    type: 'terms',
    content: fallbackTermsText,
    version: 1,
    is_published: true
  }]);

  if (e2) console.error("Error inserting terms:", e2);
  else console.log("Terms inserted.");
